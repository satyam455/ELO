...
contract Ownable is Context {
    ...
    function getUnlockTime() public view returns (uint256) { //@note get corrected
        return _lockTime;
    }
...
contract ELOToken is Context, IERC20, IERC20Metadata, Ownable {
     ...
     * construction.
     * 
     * sets metadata, mints total supply and distributes allocation wallets. //@note 
     */
    constructor () {
        _name = "Effortless Order";
        _symbol = "ELO";
        _decimals = 18;
        _totalSupply = 100 * 10**7 * 10**18;  // Set total supply to 1 billion tokens //@note
        ...
        transfer(cexReservedAddress, _cexReservedAmount);
        ...
        transfer(teamaddress, _teamAmount); //@note code review 
    }
     ...
     * - `sender` must have a balance of at least `amount`.
     * 
     * Subtract from sender adds to rceipient and emits transfer event //@note
     */
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance"); //@note Verify sender has sufficient balance
        _balances[sender] = senderBalance - amount; //@note Deduct tokens from sender
        _balances[recipient] += amount; //@note Credit tokens to recipient
     ...
     * 
     * increase balance and emits transfer event only allowed minting is permanently complete. //@note 
     */
    function _mint(address account, uint256 amount) internal virtual {
        require(!mintingFinishedPermanent,"cant be minted anymore!"); //@note Prevent minting after initial supply creation
        require(account != address(0), "ERC20: mint to the zero address");

        _beforeTokenTransfer(address(0), account, amount);

        //_totalSupply += amount;
        _balances[account] += amount; //@note Credit tokens to recipient account
        emit Transfer(address(0), account, amount);//@note Emit event with zero address as sender
    } 
